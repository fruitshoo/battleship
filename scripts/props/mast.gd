...
func _process(_delta: float) -> void:
	...
	if not is_instance_valid(wind_manager): return
	
	# 바람 방향에 따른 돛대 회전은 배 본체(player_ship.gd, chaser_ship.gd)에서 _auto_adjust_sail 등을 통해 
	# sail_angle 변수를 직접 설정하도록 위임하거나, 
	# 독립적으로 돛대가 스스로 최적의 각도를 찾는 로직을 넣을 수 있습니다.
	# 현재는 기존 player_ship.gd/chaser_ship.gd의 구조를 최대한 유지하도록 속성만 제공
