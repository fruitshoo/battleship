...
static func apply_layout_density(hud) -> void:
	...
	if is_instance_valid(hud.player_status_root):
		var player_status_height: float = player_status_hp_height + player_status_speed_height + float(hud.PLAYER_STATUS_BAR_GAP)
...
static func setup_bottom_right_layout(hud) -> void:
	...
	if _is_item_hud_enabled() and hud.item_bar == null:
		hud.item_bar = HudItemBar.new()
		hud.bottom_right_container.add_child(hud.item_bar)

static func _is_item_hud_enabled() -> bool:
	if is_instance_valid(UpgradeManager) and UpgradeManager.has_method("is_item_system_enabled"):
		return UpgradeManager.is_item_system_enabled()
	return true
...
static func setup_boss_hp_bar(hud) -> void:
	...
	_apply_boss_hp_stack_layout(hud, 560.0, 16.0, 152.0, 7.0, 24.0, 0)
