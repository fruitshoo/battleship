...
func _build_compare_overlay_text() -> String:
	...
	return "scenario:%s phase:%s\nbase fps:%d avg:%.2fms | overlay fps:%d avg:%.2fms | delta:%.2fms\nstat:%s distance:%s deck_light:%s ships:%d soldiers:%d projectiles:%d" % [
		...
		"ON" if baseline.get("stat_enabled", false) == true or overlay.get("stat_enabled", false) == true else "OFF",
		"ON" if baseline.get("distance_enabled", false) == true or overlay.get("distance_enabled", false) == true else "OFF",
		"ON" if overlay.get("deck_light_enabled", false) == true or baseline.get("deck_light_enabled", false) == true else "OFF",
		_count_ships(),
		_count_soldiers(),
		_count_projectiles(),
	]
