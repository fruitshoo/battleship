@tool
extends "res://scripts/test/ai_ship_isolation_process_ai.gd"
...
func capture_ship() -> void:
	...
	if EntityRegistry.count_legacy_captured_ships() >= 1:
...
func _process_support_ai(delta: float) -> void:
	AIShipSupportHelper.process_support_ai(self, delta)
