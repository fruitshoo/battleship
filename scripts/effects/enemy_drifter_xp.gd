...
@export var bonus_xp_amount: int = 0
...
func configure(next_xp_amount: int, next_soldier_count: int = 1, next_bonus_xp_amount: int = 0) -> void:
	xp_amount = max(0, next_xp_amount)
	bonus_xp_amount = max(0, next_bonus_xp_amount)
...
func _grant_reward(player_ship: Node3D) -> void:
	...
	if is_instance_valid(_cached_lm) and bonus_xp_amount > 0 and _cached_lm.has_method("add_bonus_xp"):
		_cached_lm.add_bonus_xp(bonus_xp_amount)
