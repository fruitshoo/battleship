...
static func _counts_as_player_roster_member(soldier: Node) -> bool:
	...
	if soldier.has_method("is_player_team_soldier") and not soldier.is_player_team_soldier():
		return false
	elif not soldier.has_method("is_player_team_soldier") and str(soldier.get("team")) != "player":
		return false
	return SoldierStateHelper.is_alive_soldier(soldier) or SoldierStateHelper.is_incapacitated_soldier(soldier)

static func _captain_retention_rank(soldier: Node) -> int:
	if SoldierStateHelper.is_alive_soldier(soldier):
		return 2
...
static func get_player_roster_count(ship) -> int:
	...
	for child in EntityRegistry.get_soldiers_by_ship(ship):
		if _counts_as_player_roster_member(child):
			count += 1
	return count
...
static func sync_player_crew_roster(ship, allow_spawn_missing: bool = true) -> void:
	...
	for child in EntityRegistry.get_soldiers_by_ship(ship):
		if not _counts_as_player_roster_member(child):
			continue
		if is_captain(ship, child):
			roster_captains.append(child)
			continue
		var role = get_soldier_role(ship, child)
		if not roster_by_role.has(role):
