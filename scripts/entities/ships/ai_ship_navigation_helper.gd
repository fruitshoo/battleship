extends RefCounted
class_name AIShipNavigationHelper
...
static func build_navigation(ship, target_node: Node3D) -> Dictionary:
	...
	if ship.get("limbo_ai_pilot_enabled") == true:
		...
		var nav_hint := ShipAIIntentHelper.get_limbo_navigation_intent(ship, target_node)
		if not boarding_nav_locked and not nav_hint.is_empty():
			if nav_hint.has(ShipAIIntentHelper.KEY_DESIRED_POINT):
				desired_point = nav_hint[ShipAIIntentHelper.KEY_DESIRED_POINT]
			if nav_hint.has(ShipAIIntentHelper.KEY_HEADING_POINT):
				heading_point = nav_hint[ShipAIIntentHelper.KEY_HEADING_POINT]
			if nav_hint.has(ShipAIIntentHelper.KEY_SPEED_MULT):
				desired_speed_mult = maxf(desired_speed_mult, float(nav_hint[ShipAIIntentHelper.KEY_SPEED_MULT]))
			if nav_hint.has(ShipAIIntentHelper.KEY_PERMIT_SPRINT):
				permit_sprint = nav_hint[ShipAIIntentHelper.KEY_PERMIT_SPRINT] == true
			var hint_mode := str(nav_hint.get(ShipAIIntentHelper.KEY_MODE, "")).strip_edges()
